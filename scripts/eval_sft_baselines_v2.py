"""Evaluate SFT checkpoints as step-0 baselines for v2 runs (max_cot_tokens=4096).

Same 12 SFT checkpoints, but now with 4096-token CoT budget to match v2 training.
Results go to logs/eval-penalty-v2/grpo-v2-{run}/Qwen_Qwen3-{size}_000000.jsonl
"""
...
def main():
    ...
    output_base = Path("logs/eval-penalty-v2")

    tasks = []
    for cond_name, (model, ckpt) in SFT_CHECKPOINTS.items():
        for seed in [42, 43]:
            run_name = f"grpo-v2-{cond_name}-s{seed}"
