"""Plot evaluation results from v4 penalty RL runs (penalty_weight=-0.5).

Reads from logs/eval-penalty-v4/ and generates Pareto + spillover plots.
"""
...
EVAL_DIR = Path("logs/eval-penalty-v4")
PLOT_DIR = Path("plots")
...
def load_all_results():
    ...
    for size in SIZES:
        for cond in CONDITIONS:
            for src in SOURCES:
                for seed in SEEDS:
                    run_name = f"grpo-v4-{size}-{cond}-{src}-s{seed}"
...
def plot_pareto(results):
    """Pareto trajectory: CoT detection (x) vs training reward (y)."""
    penalty_weight = -0.5
    N = 378
    ...
    for idx, size in enumerate(SIZES):
        ...
        for cond in CONDITIONS:
            for src in SOURCES:
                ...
                y_err = [
                    1.96 * np.sqrt(max(s * (1 - s), 0) / N + penalty_weight**2 * max(o * (1 - o), 0) / N)
    ...
    fig.suptitle("V4 (penalty_weight=-0.5): Output Penalty Spillover", fontsize=15, fontweight="bold", y=1.02)
    plt.tight_layout()
    plt.savefig(PLOT_DIR / "v4_pareto_cot_vs_output.png", dpi=300, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v4_pareto_cot_vs_output.png'}")
...
def plot_spillover_curves(results):
    ...
    fig.suptitle("V4 Spillover Curves (penalty_weight=-0.5, solid=Qwen, dashed=Haiku)", fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(PLOT_DIR / "v4_spillover_curves.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v4_spillover_curves.png'}")
...
def plot_sycophancy_and_correctness(results):
    ...
    for col, cond in enumerate(CONDITIONS):
        for row, size in enumerate(SIZES):
            ...
            for src, ls, alpha in [("qwen", "-", 1.0), ("haiku", "--", 0.7)]:
                ...
                ax.plot(steps_x, syc_means, f"o{ls}", color="#9b59b6", alpha=alpha,
                        label=f"Sycophancy ({src})", linewidth=1.5)
                ax.plot(steps_x, cor_means, f"s{ls}", color="#f39c12", alpha=alpha,
                        label=f"Real correct ({src})", linewidth=1.5)
    ...
    fig.suptitle("V4 Sycophancy & Real Correctness (penalty_weight=-0.5, solid=Qwen, dashed=Haiku)",
                 fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(PLOT_DIR / "v4_sycophancy_correctness.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v4_sycophancy_correctness.png'}")
...
def main():
    ...
    plot_sycophancy_and_correctness(results)
    print("\nAll v4 plots saved to plots/")
