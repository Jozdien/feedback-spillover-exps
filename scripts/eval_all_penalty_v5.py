"""Launch evaluations on all v5 penalty RL checkpoints (penalty_weight=-0.5, no_answer_penalty=-1.0).

Reads checkpoints.jsonl from each grpo-v5-* run directory, skips already-evaluated
checkpoints, and launches evals with controlled concurrency.

Usage:
    uv run scripts/eval_all_penalty_v5.py --max-concurrent 4
    uv run scripts/eval_all_penalty_v5.py --max-concurrent 4 --dry-run
"""
...
PENALTY_RUNS = [
    f"grpo-v5-{size}-{cond}-{src}-s{seed}"
    for size in ["8b", "32b"]
...
def get_eval_tasks(output_base: Path):
    tasks = []
    for run_name in PENALTY_RUNS:
        run_dir = Path("logs") / run_name
        ckpt_file = run_dir / "checkpoints.jsonl"
        if not ckpt_file.exists():
            print(f"  Skipping {run_name}: no checkpoints.jsonl")
            continue
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-penalty-v5")
