#!/bin/bash
# Launch 24 penalty RL runs v2: same as v1 but with max_thinking_tokens=4096
# Single-adapter GRPO with output pirate penalty (baseline condition)
# 1000 batches each (num_episodes=12000, batch_size=12)

set -a && source .env && set +a

COMMON="task=qa penalty_weight=-2 num_episodes=12000 save_every=100 max_thinking_tokens=4096"
DELAY=60  # seconds between launches

# --- SFT checkpoint paths (same as v1) ---
...
launch() {
    ...
    local log_dir="logs/grpo-v2-${name}-s${seed}"
    local log_file="logs/grpo-v2-${name}-s${seed}.log"
...
echo "=== Launching 24 penalty RL runs (v2, max_thinking_tokens=4096) ==="
...
echo "=== All 24 v2 penalty RL runs launched ==="
