"""Output hint vs CoT hint: per-model, all conditions overlaid.

Each subplot shows both out_hint (dashed) and cot_hint (solid) for each condition,
demonstrating that output penalty drives both metrics down (spillover), while
control preserves both, and RT zeroes output but leaves CoT partially intact.
"""
...
models = {
    "Qwen3-8B": {
        ...
        "pirate": {
            "steps": [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000],
            "cot": [0.322, 0.184, 0.070, 0.021, 0.010, 0.002, 0.011, 0.003, 0.005, 0.004, 0.001],
            "out": [0.381, 0.219, 0.068, 0.020, 0.013, 0.012, 0.010, 0.003, 0.007, 0.005, 0.001],
        },
    },
    "Qwen3-32B": {
        "penalty": {
...
for ax, (model_name, conditions) in zip(axes, models.items()):
    for cond_name, data in conditions.items():
        ...
        cot_ses = [1.96 * np.sqrt(p * (1 - p) / N) for p in data["cot"]]
        out_ses = [1.96 * np.sqrt(p * (1 - p) / N) for p in data["out"]]

        h_cot = ax.errorbar(
            steps, data["cot"], yerr=cot_ses,
            color=color, linewidth=2.5, linestyle="-", marker="o",
            markersize=6, capsize=3, zorder=3, alpha=0.9,
        )
        h_out = ax.errorbar(
            steps, data["out"], yerr=out_ses,
            color=color, linewidth=1.5, linestyle="--", marker="s",
            markersize=4, capsize=2, zorder=2, alpha=0.6,
        )
...
legend_elements = []
for cond_name in ["penalty", "control", "rt", "mf", "tmf", "pirate"]:
    legend_elements.append(Line2D([0], [0], color=cond_colors[cond_name], lw=2.5,
                                   label=f"{cond_labels[cond_name]} — CoT", linestyle="-", marker="o", markersize=6))
    legend_elements.append(Line2D([0], [0], color=cond_colors[cond_name], lw=1.5,
                                   label=f"{cond_labels[cond_name]} — Output", linestyle="--", marker="s", markersize=4, alpha=0.6))
...
fig.suptitle(
    "Output Hint & CoT Hint Across Conditions\n(solid=CoT spillover, dashed=output hint)",
    fontsize=15, fontweight="bold", y=1.00,
)

plt.tight_layout(rect=[0, 0.06, 1, 0.96])
plt.savefig("plots/out_vs_cot_conditions.png", dpi=300, bbox_inches="tight")
print("Saved to plots/out_vs_cot_conditions.png")
