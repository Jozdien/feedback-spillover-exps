"""Launch evaluations on all v4 penalty RL checkpoints (penalty_weight=-0.5).

Reads checkpoints.jsonl from each grpo-v4-* run directory, skips already-evaluated
checkpoints, and launches evals with controlled concurrency.

Usage:
    uv run scripts/eval_all_penalty_v4.py --max-concurrent 4
    uv run scripts/eval_all_penalty_v4.py --max-concurrent 4 --dry-run
"""
...
PENALTY_RUNS = [
    f"grpo-v4-{size}-{cond}-{src}-s{seed}"
    for size in ["8b", "32b"]
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-penalty-v4")
