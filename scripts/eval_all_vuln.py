"""Launch evaluations on the CoT-pressure "vulnerability" run checkpoints.

Covers all within-env QA arms: grpo-vuln-{C,A,A3,A0,R}-8b-s{seed}. (They are all
QA-with-hints runs, so eval_grpo_baseline.py — which loads MMLU — applies to all.)
...
Usage:
    uv run scripts/eval_all_vuln.py --max-concurrent 8
    uv run scripts/eval_all_vuln.py --max-concurrent 8 --dry-run
"""
...
RUN_PREFIXES = ["grpo-vuln-"]  # all within-env vuln arms are QA (C/A/A3/A0/R)


def discover_runs():
    """Find vuln QA run directories that have checkpoints.jsonl."""
    logs = Path("logs")
    runs = []
    for d in sorted(logs.iterdir()):
        if not d.is_dir():
            continue
        name = d.name
        if not any(name.startswith(p) for p in RUN_PREFIXES):
            continue
...
def get_eval_tasks(output_base: Path):
    ...
    for run_name in discover_runs():
        ...
        size = "32b" if "-32b-" in run_name else "8b"
...
def run_eval(task):
    cmd = [
        ...
        "--batch-size", "50",
        "--max-cot-tokens", "300",  # match training budget (vuln runs train at thinking=300)
    ]
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-vuln")
