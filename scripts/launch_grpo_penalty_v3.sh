#!/bin/bash
# Launch 24 penalty RL runs v3: same as v2 but with penalty_weight=-1 (instead of -2)
# Single-adapter GRPO with output pirate penalty
# 1000 batches each (num_episodes=12000, batch_size=12)

set -a && source .env && set +a

COMMON="task=qa penalty_weight=-1 num_episodes=12000 save_every=100 max_thinking_tokens=4096"
DELAY=60  # seconds between launches

# --- SFT checkpoint paths (same as v1/v2) ---
...
launch() {
    ...
    local log_dir="logs/grpo-v3-${name}-s${seed}"
    local log_file="logs/grpo-v3-${name}-s${seed}.log"
...
echo "=== Launching 24 penalty RL runs (v3, penalty_weight=-1) ==="
...
echo "=== All 24 v3 penalty RL runs launched ==="
