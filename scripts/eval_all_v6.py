"""Launch evaluations on all v6/v6ctrl GRPO checkpoints (Alpaca pirate-output SFT).

Reads checkpoints.jsonl from each grpo-v6-*/grpo-v6ctrl-* run directory,
skips already-evaluated checkpoints, and launches evals with controlled concurrency.

Usage:
    uv run scripts/eval_all_v6.py --max-concurrent 8
    uv run scripts/eval_all_v6.py --max-concurrent 8 --dry-run
"""
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-penalty-v6")
