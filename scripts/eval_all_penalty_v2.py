"""Launch evaluations on all v2 penalty RL checkpoints (max_thinking_tokens=4096).

Reads checkpoints.jsonl from each grpo-v2-* run directory, skips already-evaluated
checkpoints, and launches evals with controlled concurrency.

Usage:
    uv run scripts/eval_all_penalty_v2.py --max-concurrent 4
    uv run scripts/eval_all_penalty_v2.py --max-concurrent 4 --dry-run
"""
...
PENALTY_RUNS = [
    f"grpo-v2-{size}-{cond}-{src}-s{seed}"
    for size in ["8b", "32b"]
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-penalty-v2")
