"""Plot evaluation results from v2 penalty RL runs (max_thinking_tokens=4096).

Reads from logs/eval-penalty-v2/ and generates Pareto + spillover plots.
"""
...
def plot_pareto(results):
    ...
    fig.suptitle("V2 (4096 CoT tokens): Output Penalty Spillover", fontsize=15, fontweight="bold", y=1.02)
    plt.tight_layout()
    plt.savefig(PLOT_DIR / "v2_pareto_cot_vs_output.png", dpi=300, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v2_pareto_cot_vs_output.png'}")
...
def plot_spillover_curves(results):
    ...
    fig.suptitle("V2 Spillover Curves (4096 CoT tokens, solid=Qwen, dashed=Haiku)", fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(PLOT_DIR / "v2_spillover_curves.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v2_spillover_curves.png'}")
