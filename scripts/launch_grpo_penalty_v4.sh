#!/bin/bash
# Launch 24 penalty RL runs v4: penalty_weight=-0.5
# Same SFT checkpoints and setup as v2/v3, weaker penalty to map the spillover curve
# 1000 batches each (num_episodes=12000, batch_size=12)

set -a && source .env && set +a

COMMON="task=qa penalty_weight=-0.5 num_episodes=12000 save_every=100 max_thinking_tokens=4096"
DELAY=60  # seconds between launches

# --- SFT checkpoint paths (same as v1/v2/v3) ---
...
launch() {
    ...
    local log_dir="logs/grpo-v4-${name}-s${seed}"
    local log_file="logs/grpo-v4-${name}-s${seed}.log"
...
echo "=== Launching 24 penalty RL runs (v4, penalty_weight=-0.5) ==="
...
echo "=== All 24 v4 penalty RL runs launched ==="
