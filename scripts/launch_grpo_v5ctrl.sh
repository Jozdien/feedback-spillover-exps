#!/bin/bash
# Launch 24 V5 control runs: penalty_weight=0, no_answer_penalty=-1.0
# Same SFT checkpoints as V5, but no output hint penalty.
# Reward = sycophancy + format only.

set -a && source .env && set +a

COMMON="task=qa penalty_weight=0 no_answer_penalty=-1.0 num_episodes=12000 save_every=100 max_thinking_tokens=4096"
DELAY=60
...
launch() {
    ...
    local log_dir="logs/grpo-v5ctrl-${name}-s${seed}"
    local log_file="logs/grpo-v5ctrl-${name}-s${seed}.log"
...
echo "=== Launching 24 V5 control runs (penalty_weight=0, no_answer_penalty=-1.0) ==="
...
echo "=== All 24 V5 control runs launched ==="
