#!/bin/bash
# Launch 24 penalty RL runs: 12 SFT checkpoints × 2 seeds
# Single-adapter GRPO with output pirate penalty (baseline condition)
# 1000 batches each (num_episodes=12000, batch_size=12)

set -a && source .env && set +a

COMMON="task=qa penalty_weight=-2 num_episodes=12000 save_every=100"
DELAY=60  # seconds between launches

# --- 8B runs (model_name=Qwen/Qwen3-8B) ---

# SFT checkpoint paths (final)
...
# SFT checkpoint paths (final) — 32B
...
launch() {
    ...
    local log_dir="logs/grpo-${name}-s${seed}"
    local log_file="logs/grpo-${name}-s${seed}.log"
...
echo "=== Launching 24 penalty RL runs ==="
...
echo "=== All 24 penalty RL runs launched ==="
