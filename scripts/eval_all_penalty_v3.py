"""Launch evaluations on all v3 penalty RL checkpoints (penalty_weight=-1).

Reads checkpoints.jsonl from each grpo-v3-* run directory, skips already-evaluated
checkpoints, and launches evals with controlled concurrency.

Usage:
    uv run scripts/eval_all_penalty_v3.py --max-concurrent 4
    uv run scripts/eval_all_penalty_v3.py --max-concurrent 4 --dry-run
"""
...
PENALTY_RUNS = [
    f"grpo-v3-{size}-{cond}-{src}-s{seed}"
    for size in ["8b", "32b"]
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-penalty-v3")
