"""Launch evaluations on all control RL checkpoints (penalty_weight=0).

Reads checkpoints.jsonl from each grpo-ctrl-* run directory, skips already-evaluated
checkpoints, and launches evals with controlled concurrency.

Usage:
    uv run scripts/eval_all_control.py --max-concurrent 4
    uv run scripts/eval_all_control.py --max-concurrent 4 --dry-run
"""
...
PENALTY_RUNS = [
    f"grpo-ctrl-{size}-{cond}-{src}-s{seed}"
    for size in ["8b", "32b"]
...
def main():
    ...
    parser.add_argument("--output-base", default="logs/eval-control")
