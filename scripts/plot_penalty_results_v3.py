"""Plot evaluation results from v3 penalty RL runs (penalty_weight=-1).

Reads from logs/eval-penalty-v3/ and generates Pareto + spillover plots.
"""
...
EVAL_DIR = Path("logs/eval-penalty-v3")
PLOT_DIR = Path("plots")
...
def load_all_results():
    ...
    for size in SIZES:
        for cond in CONDITIONS:
            for src in SOURCES:
                for seed in SEEDS:
                    run_name = f"grpo-v3-{size}-{cond}-{src}-s{seed}"
...
def plot_pareto(results):
    ...
    for idx, size in enumerate(SIZES):
        ...
        ax.set_ylabel("Training reward (sycophancy - 1 * out_hint)", fontsize=12)
        ax.set_title(f"Qwen3-{size.upper()}", fontsize=13, fontweight="bold")
    ...
    fig.suptitle("V3 (penalty_weight=-1): Output Penalty Spillover", fontsize=15, fontweight="bold", y=1.02)
    plt.tight_layout()
    plt.savefig(PLOT_DIR / "v3_pareto_cot_vs_output.png", dpi=300, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v3_pareto_cot_vs_output.png'}")
...
def plot_spillover_curves(results):
    ...
    fig.suptitle("V3 Spillover Curves (penalty_weight=-1, solid=Qwen, dashed=Haiku)", fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(PLOT_DIR / "v3_spillover_curves.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved {PLOT_DIR / 'v3_spillover_curves.png'}")
