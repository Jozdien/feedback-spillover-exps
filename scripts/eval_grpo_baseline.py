"""Evaluate base models on MMLU QA with hints — measure baseline hint rates before training."""
...
async def evaluate(
    ...
    if checkpoint:
        tc = service.create_training_client_from_state_with_optimizer(checkpoint)
        sampler_path = tc.save_weights_for_sampler(name="eval").result().path
        sp = service.create_sampling_client(model_path=sampler_path)
        label = f"{model_name} @ {checkpoint.split('/')[-1]}"
    else:
    ...
    ckpt_slug = checkpoint.split("/")[-1] if checkpoint else "base"
    model_slug = model_name.replace("/", "_")
    out_file = out_dir / f"{model_slug}_{ckpt_slug}.jsonl"
    of = open(out_file, "w")
    of.write(json.dumps({
        "type": "metadata",
        "model": model_name,
...
def main():
    ...
    import nest_asyncio
    nest_asyncio.apply()
    asyncio.run(evaluate(args.model, args.checkpoint, args.max_questions, args.batch_size, args.output_dir))
